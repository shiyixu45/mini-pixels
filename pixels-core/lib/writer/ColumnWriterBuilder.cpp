...
std::shared_ptr<ColumnWriter> ColumnWriterBuilder::newColumnWriter(std::shared_ptr<TypeDescription> type,
	...
	case TypeDescription::LONG:
		//            return
		//            std::dynamic_pointer_cast<ColumnWriter,IntegerColumnWriter>(std::make_shared<IntegerColumnWriter>(type,
		//            writerOption));
	...
	case TypeDescription::STRUCT:
		break;
	default:
		throw InvalidArgumentException("bad column type in ColumnWriterBuilder: " +
